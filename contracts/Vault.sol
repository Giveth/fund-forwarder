...
contract Vault is Escapable, Ownable {
...
///////////////////////////////
    ...
    function authorizeSpender(
        ...
        spenderAddresses.length = SafeMath.add(spenderAddresses.length, 1);
    ...
    function unauthorizeSpender(address _spender) onlyOwner {
        ...
        spenderAddresses.length = SafeMath.sub(spenderAddresses.length, 1);
...
/////////
    ...
    /// @notice Basic fallback function; creates event and accepts funds
    function () payable {
        uint amount;
        amount = msg.value;
...
////////
    ...
    function authorizePayment(
        ...
        authorizedPayments.length = SafeMath.add(authorizedPayments.length, 1);
        ...
        if (_paymentDelay >= timeLock) {
            p.earliestPayTime = SafeMath.add(now, _paymentDelay);
        } else {
            p.earliestPayTime = SafeMath.add(now, timeLock);
        }
        ...
        totalAuthorizedToBeSpent = SafeMath.add(totalAuthorizedToBeSpent, p.amount);
        ...
        if ((now >= p.earliestPayTime) && (getBalance() >= p.amount)) {
            ...
            totalAuthorizedToBeSpent = SafeMath.sub(totalAuthorizedToBeSpent, p.amount);
            totalSpent = SafeMath.add(totalSpent, p.amount);         // Accounting
    ...
    function collectAuthorizedPayment(uint _idPayment) {
        ...
        totalAuthorizedToBeSpent = SafeMath.sub(totalAuthorizedToBeSpent, p.amount);
        totalSpent = SafeMath.add(totalSpent, p.amount);
...
/////////
    ...
    function delayPayment(uint _idPayment, uint _delay) onlySecurityGuard {
        ...
        if ((SafeMath.add(p.securityGuardDelay, _delay) > maxSecurityGuardDelay) ||
            (p.paid) ||
            (p.canceled))
            throw;

        p.securityGuardDelay = SafeMath.add(p.securityGuardDelay, _delay);
        p.earliestPayTime  = SafeMath.add(p.earliestPayTime, _delay);
